...
'''
Author List     :Utkarsh Verma, Sagnik Bhowmick, Sahib Majithia, Nikhil Kumar Mishra 
Filename        :task5_main.py
Functions       :heuristic(a,b) , astar(array, start, goal) , path_planner(start,end,grid) , main(image_filename) , get_perspective_image(frame)
Global Variables:xbee
'''
...
Input           :a frame of the video taken by the overhead camera 
Output          :gives the extracted image of the working area and door area. 
Logic           :snippet of the code was given as resources
'''
...
def get_perspective_image(frame):
	...
	pts1 = np.float32([[x3,y3],[x1,y1],[x4,y4],[x2,y2],]) 
	pts2 = np.float32([[0,0],[0,480],[680,0],[680,480]])
	persM = cv2.getPerspectiveTransform(pts1,pts2)
	img = cv2.warpPerspective(frame,persM,(680,480))
	return img
...
def main(image_filename):
		...
		******DO NOT EDIT name of these argument*******
		...
		##this loop devides our image into 45 pieces and then evaluate properties of each
		for i in range (6):
				for j in range (9):
						img1=image[(((j*x)/9)):((((j+1)*x)/9)),(((i*y)/6)):((((i+1)*y)/6)),:]
...
if __name__ == '__main__':
	...
	main(image_filename)
		xbee.remote_at(
			dest_addr='\x56\x78',
			command= 'data', #where data is the function to be transmitted
			parameter='\x04')
